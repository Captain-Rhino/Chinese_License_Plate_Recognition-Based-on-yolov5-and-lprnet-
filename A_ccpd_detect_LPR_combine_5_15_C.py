...
import A_LPR_class_5_16_C

@smart_inference_mode()
def run(
    weights=ROOT / "half_ccpd.pt",  # model path or triton URL
    source=ROOT / "data/ccpd_test_img",  # file/dir/URL/glob/screen/0(webcam)
    ...
    lpr = A_LPR_class_5_16_C.LicensePlateRecognizer
    source = str(source)
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_crop:
                        ...
                        crop_return = save_one_box(xyxy, imc, file=save_dir / "crops" / names[c] / f"{p.stem}.jpg",
                            ...
                            ###判定颜色 终止
                        #print(crop_return)
                        predicted_labels = lpr.recognize_plate(crop_return)
                    ...
                    if save_img or view_img:  # Add bbox to image
                        c = int(cls)  # integer class
                        label = None if hide_labels else (names[c] if hide_conf else f"{predicted_labels} {conf:.2f}")
